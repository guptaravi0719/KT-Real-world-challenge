...
class VehicleDataView:
    ...
    class DiscontinuedVehicleViewData(APIView):
        def get(self,request):
            data = self.getDiscuntinuedVehicles()
            return JsonResponse(data, safe = False)
        
        def getModelsFromYear(self, year):
        ...
        def getDiscuntinuedVehicles(self):
            models_10_yr = set()
            models_2_yr = set()
            all_models = {}
            ...
            for i in range(len(years)):
                result = self.getModelsFromYear(years[i])
